...
impl BatchExecutor {
        ...
        A: TxApi<'a, T>,
    {
        ...
        if rslt_p1.is_err() {
            // warn!("[Result P1 error] Updating the transfer!!! \n\n\n {batch:#?}");
        ...
        if rslt_p2.is_err() {
            // warn!("[Result P2 error] Updating row version!!! \n\n\n {batch:#?}");
        ...
        if let Some(new_version) = snapshot {
            ...
            if let Ok(rows) = snapshot_update_result {
                ...
                // warn!(
                //     "[Snapshot version update error] Updating snapshot version to={new_version} from current_snapshot={current_snapshot}!!! \n\n\n {batch:#?}"
                // );
...
//             async fn query_one<T>(&self, sql: &str, params: &[&(dyn ToSql + Sync)], fn_converter: fn(&Row) -> T) -> T;
