...
// Selectively deserialising `on_commit` actions using `Value::deserialize` is still expensive on very high payload.
// Instead if deserialization fails, catch the error and skip inserting the candidate to suffix.
