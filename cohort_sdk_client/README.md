...
const response = initiator.certify(fnMakeNewRequestCallback, fnOooInstaller).await

// TODO: Implement and document response handling
```
...
### The Initiator API - "New Request Callback"
...
### The Initiator API - "Out of Order Install Callback"
...
| `backoffOnConflict.maxMs` | The re-try logic implemented in SDK uses exponential backoff strategy. The delay will grow anywhere between min and max values. | 1500 |
...
| `retryBackoff.maxMs` | The re-try logic implemented in SDK uses exponential backoff strategy. The delay will grow anywhere between min and max values. | 1500 |
| `retryAttemptsMax` | This parameter comes into play during re-try logic implemented in SDK. How many times to re-try the certification request. | 10 |
...
| `retryOoBackoff.maxMs` | The re-try logic implemented in SDK uses exponential backoff strategy. The delay will grow anywhere between min and max values. | 1000 |
...
| `agent` | Cohort Agent Name, this must be unique per cohort instance. Passed in the header `certAgent` in kafka message. Used to correlate response with your certification request. | |
...
| `kafka.groupId` | The group id of cohort topic consumer. | |
